CREATE PROCEDURE [neo].[GetWebSiteDispensePerPour] 
	@EDISID						INT,
	@From						DATETIME,
	@To							DATETIME,
	...
	--@IncludeMetric				BIT,
	@Pump						INT = NULL,
	@TemperatureAmberValue		FLOAT = 2, --check
	@UnderSpecIsInSpec			BIT = 1	,
	@IncludeUnknownLiquidTypes	BIT = 0 --check
...
AS
	...
	Pints AS Volume,
	Pump + PumpOffset AS Tap,
	Duration,
	EstimatedDrinks AS Drinks,
	AverageTemperature AS Temperature,
	CASE 
	...
	AND (Products.ID IN ( SELECT * FROM @ProductIDs) OR @ProductIDsCount = 0)
	ORDER BY StartTime, Pump+PumpOffset

GO
GRANT EXECUTE
    ON OBJECT::[neo].[GetWebSiteDispensePerPour] TO PUBLIC
    AS [dbo];

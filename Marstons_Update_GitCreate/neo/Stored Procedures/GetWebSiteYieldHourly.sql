CREATE PROCEDURE [neo].[GetWebSiteYieldHourly]
(
	@EDISID							INT,
	@From							DATE,
	@To							DATE,
	...
	-- No longer needed for Neo Portal - leaving in for now as requirements may change...
	--@IncludeCasks					BIT,
	--@IncludeKegs					BIT,
	--@IncludeMetric					BIT,
...
WHERE (ValidFrom <= @To)
AND (ISNULL(ValidTo, @From) >= @From)
AND (ISNULL(ValidTo, @From) >= @SiteOnline)
...
-- may not be needed - cross- checking data
--AND NOT EXISTS
--(
--	SELECT ID
...
(TradingDate, ProductID, Quantity)
SELECT DATEADD(Hour, DATEPART(Hour, [SaleTime]), TradingDate) AS [TradingDate],
...
--		AND @ExcludeServiceIssues = 1
--)
GROUP BY DATEADD(Hour, DATEPART(Hour, [SaleTime]), TradingDate), ISNULL(PrimaryProducts.PrimaryProductID, Sales.ProductID)

-- Calculate daily yield
SELECT 
	COALESCE(BeerDispensed.[TradingDate], BeerSold.[TradingDate], CleaningWaste.[TradingDate]) AS [TradingDate],
	DATEPART(Hour, COALESCE(BeerDispensed.[TradingDate], BeerSold.[TradingDate], CleaningWaste.[TradingDate])) AS [TradingHour],
	--DATEPART(Hour, COALESCE(BeerDispensed.[TradingDate], BeerSold.[TradingDate], CleaningWaste.[TradingDate], NumberOfLinesCleaned.[TradingDate])) AS [TradingHour],
	...
	--DATEPART(hour,(CASE WHEN DATEPART(Hour, COALESCE(BeerDispensed.[TradingDate], BeerSold.[TradingDate], CleaningWaste.[TradingDate], NumberOfLinesCleaned.[TradingDate])) < 5 
	-- THEN DATEADD(Day, +1, COALESCE(BeerDispensed.[TradingDate], BeerSold.[TradingDate], CleaningWaste.[TradingDate], NumberOfLinesCleaned.[TradingDate])) 
	-- ELSE COALESCE(BeerDispensed.[TradingDate], BeerSold.[TradingDate], CleaningWaste.[TradingDate], NumberOfLinesCleaned.[TradingDate]) 
	--END))+ 1 AS Shift,
...
--)
ORDER BY CASE WHEN DATEPART(HOUR,COALESCE(BeerDispensed.[TradingDate], BeerSold.[TradingDate], CleaningWaste.[TradingDate])) < 5 THEN 1 ELSE 0 END,
		 COALESCE(BeerDispensed.[TradingDate], BeerSold.[TradingDate], CleaningWaste.[TradingDate]), Products.Description
...
GO
GRANT EXECUTE
    ON OBJECT::[neo].[GetWebSiteYieldHourly] TO PUBLIC
    AS [dbo];

CREATE PROCEDURE [neo].[GetCustomerSitesAndExceptionsForUser]
(
                @UserID     INT,
...
BEGIN
    ...
    END
                ...
                IF (@ToCurrentDate = 1)
                                ...
                                SELECT @DatabaseID, se.EDISID, se.ID, se.[Type], se.[TypeID], se.TradingDate, se.Value, CAST(se.LowThreshold AS VARCHAR) AS 'LowThreshold', 
                                                                CAST(se.HighThreshold AS VARCHAR) AS 'HighThreshold', se.SiteDescription, se.[DateFormat], se.EmailReplyTo
                ...
                ELSE
                                ...
                                WHERE 
                                                EDISID IN (SELECT EDISID FROM @UserSites)
                                                AND (se.TradingDate >= CONVERT(date, @TradingDate) AND se.TradingDate < DATEADD(d, 1, CONVERT(date, @TradingDate)))
                                ORDER BY se.TradingDate ASC

END
GO
GRANT EXECUTE
    ON OBJECT::[neo].[GetCustomerSitesAndExceptionsForUser] TO [fusion]
    AS [dbo];

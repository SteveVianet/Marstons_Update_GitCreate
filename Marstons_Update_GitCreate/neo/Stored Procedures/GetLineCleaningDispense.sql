CREATE PROCEDURE [neo].[GetLineCleaningDispense](
	@EDISID		INT,
	@From			DATETIME ,
	@To			DATETIME,
	@WaterThreshold	FLOAT = 1,
	@IncludeNIU BIT = 0
	)
...
SELECT CONVERT(DATETIME, FLOOR(CONVERT(FLOAT, TradingDateAndTime))) AS [Date],
       TradingDispensed.Pump AS SitePump,
       Product,
	   ...
	   ISNULL(CASE WHEN CleaningProgress.Pump is not null THEN 1 END,0) AS Audited
FROM @TradingDispensed AS TradingDispensed
JOIN Sites ON Sites.EDISID = TradingDispensed.EDISID
left JOIN CleaningProgress on (TradingDispensed.EDISID = CleaningProgress.EDISID and TradingDispensed.Pump = CleaningProgress.Pump and TradingDispensed.TradingDateAndTime = CleaningProgress.[Date])
GROUP BY CONVERT(DATETIME, FLOOR(CONVERT(FLOAT, TradingDateAndTime))),
	      TradingDispensed.Pump,
	      Product,
	      Sites.Quality,
		  ISNULL(CASE WHEN CleaningProgress.Pump is not null THEN 1 END,0)
...
GO
GRANT EXECUTE
    ON OBJECT::[neo].[GetLineCleaningDispense] TO PUBLIC
    AS [dbo];

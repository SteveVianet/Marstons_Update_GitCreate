CREATE PROCEDURE [dbo].[GetOverallYieldSummary]
(
       @EDISID                           INT = NULL,
...
--DECLARE @EDISID INT = 8
--DECLARE @LocalTimeNow DATETIME = '2017-08-20 00:00:00'
...
CREATE TABLE #OverallYield(EDISID INT, Product VARCHAR(100), [Percent] FLOAT, IsCask BIT, Sold FLOAT, CashValue FLOAT, BeerMeasured FLOAT)
...
CREATE TABLE #SiteYield(EDISID INT, ProductID INT, Product VARCHAR(100), QuantityDispensed FLOAT, DrinksDispensed FLOAT, WastedDispense FLOAT, OperationalYield FLOAT, Sold FLOAT, RetailYield FLOAT, OverallYield FLOAT, IsCask BIT, MinPouringYield FLOAT, MaxPouringYield FLOAT, LowPouringYieldErrThreshold FLOAT, HighPouringYieldErrThreshold FLOAT, POSYieldCashValue FLOAT, CleaningCashValue FLOAT, PouringYieldCashValue FLOAT, BeerInLineCleaning FLOAT)
...
(EDISID, ProductID, Product, QuantityDispensed, DrinksDispensed, WastedDispense, OperationalYield, Sold, RetailYield, OverallYield, IsCask, MinPouringYield, MaxPouringYield, LowPouringYieldErrThreshold, HighPouringYieldErrThreshold, POSYieldCashValue, CleaningCashValue, PouringYieldCashValue, BeerInLineCleaning)
SELECT  COALESCE(DispenseActions.EDISID, Sales.EDISID) AS EDISID,
        ...
        ISNULL(SUM(DispenseActions.BeerInLineCleaning),0) AS [WastedDispense],
		...
		ISNULL(SUM(Sales.Quantity),0) - (ISNULL(SUM(DispenseActions.Pints),0) + ISNULL(SUM(DispenseActions.BeerInLineCleaning),0)) AS OverallYield,
...
INSERT INTO #OverallYield
(EDISID, Product, [Percent], IsCask, Sold, CashValue, BeerMeasured)
SELECT @PrimaryEDISID,
       Product,
       (SUM(Sold) / CASE WHEN SUM(QuantityDispensed) + SUM(WastedDispense) = 0 THEN 1 ELSE SUM(QuantityDispensed) + SUM(WastedDispense) END) * 100,
       IsCask,
       SUM(Sold),
       POSYieldCashValue,
       SUM(QuantityDispensed) + SUM(WastedDispense)
FROM #SiteYield
WHERE IsCask = 0
...
SELECT EDISID, Product, [Percent], IsCask, Sold, CashValue, BeerMeasured
FROM #OverallYield
...
DROP TABLE #OverallYield
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[GetOverallYieldSummary] TO PUBLIC
    AS [dbo];

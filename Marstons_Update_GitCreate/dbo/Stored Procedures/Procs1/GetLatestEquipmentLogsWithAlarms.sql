CREATE PROCEDURE [dbo].[GetLatestEquipmentLogsWithAlarms]
(
	@EDISID					INT,
	@From					DATETIME,
	@To						DATETIME,
...
EquipmentItems.InputID,
	...
	CASE WHEN CAST(DATEADD(Second, -DATEPART(Second, DATEADD(Minute, -DATEPART(Minute, EquipmentReadings.TradingDate), EquipmentReadings.TradingDate)), DATEADD(Minute, -DATEPART(Minute, EquipmentReadings.TradingDate), EquipmentReadings.TradingDate)) AS TIME) >= EquipmentItems.AlarmStartTime 
                AND
              CAST(DATEADD(Second, -DATEPART(Second, DATEADD(Minute, -DATEPART(Minute, EquipmentReadings.TradingDate), EquipmentReadings.TradingDate)), DATEADD(Minute, -DATEPART(Minute, EquipmentReadings.TradingDate), EquipmentReadings.TradingDate)) AS TIME) <= EquipmentItems.AlarmEndTime
         THEN Temperatures.InAlarm 
         ELSE 0 END AS InAlarmThreshold
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[GetLatestEquipmentLogsWithAlarms] TO PUBLIC
    AS [dbo];

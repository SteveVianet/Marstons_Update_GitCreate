CREATE PROCEDURE [dbo].[GetStackDispense]
(
    @EDISID INT,
    @PumpID INT = NULL,
    @From   DATE,
...
--DECLARE @EDISID INT = 4
--DECLARE @From DATE  = '2016-08-02'
--DECLARE @To DATE    = '2016-08-02'
...
JOIN ( -- The Pump Setup might have been altered during the day, quickly scan the relevant tables to find all possible values
    ...
    WHERE
        [MD].[EDISID] = @EDISID
    AND [MD].[Date] BETWEEN @From AND @To
    AND (@PumpID IS NULL OR [D].[Pump] = @PumpID)
    UNION
    SELECT DISTINCT
        [MD].[EDISID],
        [W].[Line] AS [Pump]
    ...
    WHERE
        [MD].[EDISID] = @EDISID
    AND [MD].[Date] BETWEEN @From AND @To
    AND (@PumpID IS NULL OR [W].[Line] = @PumpID)
    UNION
    SELECT DISTINCT
        [MD].[EDISID],
        [C].[Line] AS [Pump]
    ...
    WHERE
        [MD].[EDISID] = @EDISID
    AND [MD].[Date] BETWEEN @From AND @To
    AND (@PumpID IS NULL OR [C].[Line] = @PumpID)
    --ORDER BY 
    --    [EDISID],
    --    [Pump]
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[GetStackDispense] TO PUBLIC
    AS [dbo];

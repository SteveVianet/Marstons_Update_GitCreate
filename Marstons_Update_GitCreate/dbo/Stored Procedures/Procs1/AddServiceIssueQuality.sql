...
CREATE PROCEDURE [dbo].[AddServiceIssueQuality]
(
	@CallID				INT = 0,
	@PumpID				INT = 0,
...
INSERT INTO ServiceIssuesQuality
...
LEFT JOIN ServiceIssuesQuality AS CurrentIssues ON CurrentIssues.EDISID = @EDISID
												AND CurrentIssues.CallID = @CallID
												AND CurrentIssues.PumpID = PumpSetup.PumpID
...
AND ValidTo IS NULL
AND ((PumpSetup.RealPumpID = @PumpID AND PumpSetup.EDISID = @EDISID) OR @ExcludeAllProducts = 1)
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[AddServiceIssueQuality] TO PUBLIC
    AS [dbo];

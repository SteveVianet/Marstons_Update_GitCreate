CREATE PROCEDURE [dbo].[GenerateAndSendVRSEmailDev]
(
	@VisitRecordID			INT,
	@SendCustomerEscalation	BIT = 1, --Send to BDM/RM (and VisitEscalationEmail in customer config)  if note is escaleted or VRSEmailAlwaysSend = 1 in customer config. Otherwise don't send anything.
	@SendCAMCopy			BIT = 0, --Send to CAM
	...
	@AdditionalRecipients	VARCHAR(500) = ''  --Send to this address(es)
)
AS
...
SELECT @DraughtDetails = @DraughtDetails 
	...
	+ '<td>' + CASE CalCheck WHEN 1 THEN 'Yes' WHEN 2 THEN 'No' ELSE 'Unknown' END  + '</td>'
...
SELECT @VolumeDetails = @VolumeDetails + Product + ' ' + CAST(DraughtVolume AS VARCHAR) + 'g, '
FROM VisitDamages
WHERE VisitRecordID = @VisitRecordID
AND DamagesType = 1

SET @VolumeDetails = LEFT(@VolumeDetails, LEN(@VolumeDetails)-1)

SELECT @StockDetails = @StockDetails + Product + ' ' + CAST(DraughtStock AS VARCHAR) + 'g, '
FROM VisitDamages
WHERE VisitRecordID = @VisitRecordID
AND DamagesType = 1

SELECT @StockDetails = @StockDetails + Product + ' ' + CAST(Cases AS VARCHAR) + 'cases, '
FROM VisitDamages
WHERE VisitRecordID = @VisitRecordID
AND DamagesType = 2
AND Cases > 0

SELECT @StockDetails = @StockDetails + Product + ' ' + CAST(Bottles AS VARCHAR) + 'bottles, '
FROM VisitDamages
WHERE VisitRecordID = @VisitRecordID
AND DamagesType = 2
AND Bottles > 0

SET @StockDetails = LEFT(@StockDetails, LEN(@StockDetails)-1)
...
JOIN @CustomerActionItems AS CustomerAction ON CustomerAction.ID = VisitRecords.BDMActionTaken
WHERE VisitRecords.[ID] = @VisitRecordID
AND Users.UserType = 9


IF NOT @EDISID IS NULL AND @ClosedByCAM = 1
--ONLY send a note out as an email if it exists (no shit) and has been submitted by the CAM
BEGIN
	...
	SET @Body = '<h1>Visit details</h1><table id="tblVisit" cellspacing="0" cellpadding="0" rules="all"  border="1"> '
				...
				+ CHAR(13) + '</tr>'
				+ '<tr>' 
					+ '<th>If met on site other - details</th><td colspan="3">' + ISNULL(@MetOnSiteOther, '') + '</td>'
				+ CHAR(13) + '</tr>'
				+ '<tr>' 
	...
	IF @DraughtDetails <> ''
	BEGIN
		SET @Body = @Body + '<h1>Draught details</h1><table id="tblDraught" cellspacing="0" cellpadding="0" rules="all"  border="1"> '
	...
	IF @PackageDetails <> ''
	BEGIN
		SET @Body = @Body + '<h1>Package details</h1><table id="tblPackage" cellspacing="0" cellpadding="0" rules="all"  border="1"> '
	...
	IF @OtherDetails <> ''
	BEGIN
		SET @Body = @Body + '<h1>Other details</h1><table id="tblPackage" cellspacing="0" cellpadding="0" rules="all"  border="1""> '
	...
	SET @Body = @Body + '<h1>Compensation summary</h1><table id="tblSummary" cellspacing="0" cellpadding="0" rules="all"  border="1"> '
	...
	---Finally we can send the emails. Hazza!
	
	--Send the ecalation email to BDM/RM if required
	...
	--Send email to additional internal address. Specified by the ID passed in to the SP.

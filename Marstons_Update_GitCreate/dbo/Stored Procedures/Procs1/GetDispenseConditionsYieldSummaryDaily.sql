CREATE PROCEDURE dbo.[GetDispenseConditionsYieldSummaryDaily]
(
	@From				DATETIME,
	@To				DATETIME,
	@EDISID			INT,
...
SELECT  DATEADD(hh, DATEPART(hh, Sales.[SaleTime]), MasterDates.[Date]),
	CASE WHEN DATEPART(Hour, [SaleTime]) < 5 THEN DATEADD(Day, -1, [Date]) ELSE [Date] END,
	SUM(Sales.Quantity)
FROM Sales
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[GetDispenseConditionsYieldSummaryDaily] TO PUBLIC
    AS [dbo];

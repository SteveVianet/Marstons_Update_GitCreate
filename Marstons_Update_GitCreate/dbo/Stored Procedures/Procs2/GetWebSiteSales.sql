CREATE PROCEDURE [dbo].[GetWebSiteSales]
(
	@EDISID						INT,
	@From						DATETIME,
	@To							DATETIME,
...
SELECT	Sales.EDISID,
		CAST(CAST(Sales.SaleDate AS DATE) AS DATETIME) + CAST(Sales.SaleTime AS TIME) AS SaleDate,
		CAST(CAST(Sales.TradingDate AS DATE) AS DATETIME) + CAST(Sales.SaleTime AS TIME) AS TradingDate,
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[GetWebSiteSales] TO PUBLIC
    AS [dbo];

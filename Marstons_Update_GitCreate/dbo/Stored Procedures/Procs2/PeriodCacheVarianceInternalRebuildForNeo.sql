CREATE 
----ALTER 
PROCEDURE [dbo].[PeriodCacheVarianceInternalRebuildForNeo]
(
	@From 					DATETIME = NULL,
	@To						DATETIME = NULL,
...
FROM Sites
WHERE 
    ((@MultipleAuditors = 0 OR @Auditor IS NULL)
     OR  
     (@MultipleAuditors = 1 AND LOWER(SiteUser) = LOWER(@Auditor)))
AND (@EDISID IS NULL OR [EDISID] = @EDISID)

IF @Debugging = 1
BEGIN
    SELECT COUNT(*) AS SitesToRegenerate FROM #Sites
END
...
IF @Debugging = 1
BEGIN
    SELECT CAST(GETDATE() - @DebugStartTime AS TIME) AS TimeToGenerate
END
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[PeriodCacheVarianceInternalRebuildForNeo] TO PUBLIC
    AS [dbo];

...
CREATE PROCEDURE [dbo].[GetWebSiteYieldHourly2]
(
	@EDISID							INT,
	@From							DATETIME,
	@To								DATETIME,
...
SELECT COALESCE(BeerDispensed.[TradingDate], BeerSold.[TradingDate], CleaningWaste.[TradingDate], NumberOfLinesCleaned.[TradingDate]) AS [TradingDateAndTime],

	CASE WHEN DATEPART(Hour, COALESCE(BeerDispensed.[TradingDate], BeerSold.[TradingDate], CleaningWaste.[TradingDate], NumberOfLinesCleaned.[TradingDate])) < 5 
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[GetWebSiteYieldHourly2] TO PUBLIC
    AS [dbo];

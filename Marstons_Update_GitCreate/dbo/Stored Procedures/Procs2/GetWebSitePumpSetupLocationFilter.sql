CREATE PROCEDURE [dbo].[GetWebSitePumpSetupLocationFilter]
(
	@EDISID		INT,
	@From			DATETIME,
	@To			DATETIME,
...
INSERT INTO @AllSitePumps (EDISID, SitePump, PumpID, LocationID, ProductID, ValidFrom, ValidTo)
...
WHERE (REPLACE(Locations.[Description], '''', '') IN (SELECT LocationDescription FROM @LocationFilter))

GO
GRANT EXECUTE
    ON OBJECT::[dbo].[GetWebSitePumpSetupLocationFilter] TO PUBLIC
    AS [dbo];

CREATE PROCEDURE [dbo].[zRS_GetCDSitesReportTOP10BDM]
...
SELECT 		 Sites.RMName
			  ...
			  ,RANK() OVER (Partition BY BDMName ORDER BY Reds.CDTY) AS [Rank]
...
WHERE Sites.EDISID NOT IN
 ...
 AND Reds.TYPeriod = @LastPeriod
 AND (Reds.CDTY IS NOT NULL AND Reds.CDTY <> 0)
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[zRS_GetCDSitesReportTOP10BDM] TO PUBLIC
    AS [dbo];

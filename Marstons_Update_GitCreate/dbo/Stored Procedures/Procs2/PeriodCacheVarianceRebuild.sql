CREATE PROCEDURE [dbo].[PeriodCacheVarianceRebuild]
(
	@From 				DATETIME = NULL,
	@To					DATETIME = NULL,
...
;WITH cte_WeekStock AS (
	...
	FROM (
		...
		WHERE (COALESCE(PrimarySites.PrimaryEDISID, MasterDates.EDISID) = @EDISID OR @EDISID IS NULL)
	...
	WHERE Ranked = 1 --AND @AccurateDeliveryProvided = 1
...
INSERT INTO PeriodCacheVariance
...
--DELIVERY AND STOCK
	...
	FROM (
		...
		FROM (
			...
			WHERE (WeekStock.EDISID = @EDISID OR @EDISID IS NULL)
	...
	LEFT JOIN (--DISPENSE BETWEEN STOCK TAKE AND END OF WEEK
		...
		FROM (
			...
			LEFT JOIN @PrimaryProducts AS PrimaryProducts ON PrimaryProducts.ProductID = Products.ID
			LEFT JOIN @PrimaryEDIS AS PrimarySites ON MasterDates.EDISID = PrimarySites.EDISID
												   --AND COALESCE(PrimarySites.PrimaryEDISID, MasterDates.EDISID) = @EDISID
			...
			--AND COALESCE(PrimaryProducts.ProductID, DLData.Product) = 18
			WHERE (
...
WHERE (Cache.EDISID = @EDISID OR @EDISID IS NULL)
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[PeriodCacheVarianceRebuild] TO PUBLIC
    AS [dbo];

CREATE PROCEDURE [dbo].[MoveCallToJobWatch]
(
    @CallID INT
)
AS
...
/* DEBUG: Invalidate one of the Reasons to "Service-5" and test whether the following can mark it as invalid */
/* MUST NOT BE ENABLED WHEN DEPLOYED */
--UPDATE @SplitReasons
--SET [JobType] = 'Service-5'
--WHERE [WorkDetail] LIKE 'Flowmeter check%'

INSERT INTO [dbo].[JobWatchCalls] 
    ([EdisID], [CallID], [JobId], [JobReference], [JobType], [OriginalJobDescription], [CurrentJobDescription], [EngineerName], [ResourceName], [JobActive], [PreRelease], [AwaitPO], [Invalid], [Posted], [PostResults], [RequestedBy])
SELECT
    ...
    1 AS [JobActive], -- While 1 we need to poll JobWatch for updates on any changes
    CASE WHEN @StatusID = 6 THEN 1 ELSE 0 END AS [PreRelease],
    CASE WHEN [POStatusID] IN (1,4) THEN 0 ELSE 1 END AS [AwaitPO], -- 1 = Not Required, 2/3 = Needed, 4 = Obtained (not possible during raise?)
	CASE WHEN [CombinedReasons].[JobType] LIKE 'Service-%' AND [CombinedReasons].[JobType] NOT IN ('Service-7','Service-14','Service-28')
		 THEN 1
		 ELSE 0
		 END AS [Invalid],
    0 AS [Posted],
    '' AS [PostResults],
    CASE WHEN [RaisedBy] LIKE 'MAINGROUP\%'
         THEN REPLACE([RaisedBy], 'MAINGROUP\', '') + '@vianetplc.com' 
         ELSE [RaisedBy]
         END AS [Converted]
FROM [dbo].[Calls]
...
EXEC [dbo].[DeleteCallCompletely] @CallID

GO
GRANT EXECUTE
    ON OBJECT::[dbo].[MoveCallToJobWatch] TO PUBLIC
    AS [dbo];
